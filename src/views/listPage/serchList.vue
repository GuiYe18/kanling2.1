<template>
  <div class="content">
    ...
    <div class="serchTitle">
      <div class="returnIcon" @click="returnHandle">
        <van-icon size="1rem" name="arrow-left" />
      </div>
      <van-search class="serchInput" v-model="searchVal" placeholder="请输入搜索关键词" />
      <div class="serchBtn" @click="serchHandle">搜索</div>
  ...
  methods: {
    ...
    judgeClassification() {
      ...
      if (this.$route.query.from === "classificationentErprise") {
        this.classification =
          JSON.parse(window.localStorage.getItem("classificationentErprise")).length === 0 ? [{ id: 1, name: "全部" }] : JSON.parse(window.localStorage.getItem("classificationentErprise"));
    ...
    toDetail(item) {
      if (item.qsntype === 1) {
        this.$router.push(this.fun.getUrl("detailEnterprise", "", { enterpriseId: item.id, receive_status: item.receive_status }));
