    ...
    <div class="bottomConten">
      ...
      <div id="content">
        ...
        <div v-if="fromBase_address == '1' && is_edit" style="margin-top: 10px" class="address_nontext">
          <div style="width: 100%; text-align: left; text-indent: 20px; background: #e3e3e3; color: #6666; padding: 5px 0">所在地信息</div>
        ...
        <div style="margin-top: 10px" v-if="is_edit && isForm">
          <div style="width: 100%; text-align: left; text-indent: 20px; background: #e3e3e3; color: #6666; padding: 5px 0">其他信息</div>
          ...
          <template v-for="(item, i) in diydata">
            <template v-if="item.type == 'diycheckbox'">
              ...
              <van-checkbox-group v-model="item.value" checked-color="#4cd864" class="select" :key="i">
                <van-cell-group :border="false" :title="item.data.tp_name">
                  <van-cell title-class="checkbox-text" v-for="(ck, i) in item.data.tp_text" :key="i" clickable :title="ck" @click="toggle(i)" :border="false" center>
                    <template #right-icon>
            ...
            <template v-if="item.type == 'diypwd'">
              <van-cell-group :border="false" class="one-line pwd-bor" :key="i">
                <van-cell title-style="text-align:left" center :border="false" style="border: solid 0.0625rem #ebedf0; border-radius: 4px">
                  <van-field
                    style="height: 30px"
            ...
            <template v-if="item.type == 'diytextarea'">
              <van-cell-group :border="false" :title="item.data.tp_name" style="margin-bottom: 10px" :key="i">
                <van-field class="more-textarea" v-model="item.value" rows="3" autosize type="textarea" maxlength="100" :placeholder="item.data.placeholder" show-word-limit />
              </van-cell-group>
            </template>
            ...
            <template v-if="item.type == 'diyimg'">
              <van-cell-group :title="item.data.tp_name" :border="false" :key="i">
                <div class="clearfloat" style="width: 100%; text-align: left; padding: 0.5rem; border: 1px solid rgb(204, 204, 204); border-radius: 4px">
