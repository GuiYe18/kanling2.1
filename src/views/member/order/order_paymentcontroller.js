...
export default {
  ...
  data() {
    return {
      ...
      // 余额积分
      balanceIntegration: this.fun.getIntegration(),
  ...
  methods: {
    ...
    // 余额支付
     confirm(btn) {
      // // 判断积分
      // if (this.Integration - this.goods_price < 0) {
      //   if (this.order_pay == 5) {
      //     Toast('爱心积分不足无法兑换')
      //     return
      //   }
      // }
      //判断通兑
      if (this.circulate - this.goods_price < 0) {
        if (this.order_pay == 6) {
          Toast('通兑不足无法兑换')
          return
        }
        ...
        .then(
          function (response) {
            if (response.result == 1) {
              ...
              that.circulate = Number(response.data.member.credit1).toFixed(0);    //通兑
              that.balance = Number(response.data.member.credit2).toFixed(2);  //余额
