  ...
  <div id="goodsinfo" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
    ...
    <div class="addr" :class="jurisdiction_set == 1 ? 'addr1' : ''">
      ...
      <ul class="addr-info">
        ...
        <template v-if="order_data.dispatch_type_id == 8 && this.fun.isTextEmpty(order_data.package_deliver)">
          <li style="margin-bottom: 0.5rem; font-size: 14px">获取收货信息出错了，请联系客服！</li>
        </template>
    ...
    <div class="goods-box">
      <div class="goods" v-for="(good, i) in order_data.has_many_order_goods" :key="i" @click="toGoodsDetail(good, order_data.is_virtual, good.is_course)">
        <div class="img">
          <img :src="good.thumb" />
        ...
        <ul class="price">
          <li class="money">
            <template v-if="brand_id == 6"> {{ good.price }} 通兑</template>
            <!-- <template v-if="brand_id == 5"> {{ good.price }} 爱心积分</template> -->
            <template v-if=" brand_id !== 6">
    ...
    <div class="tbs">
      ...
      <ul class="Sub">
        ...
        <li class="right" v-if="!(order_data.plugin_id == 40)">
          ...
          <template v-if="brand_id == 6"> {{ order_data.order_goods_price }} 通兑</template>
          <!-- <template v-if="brand_id == 5"> {{ order_data.order_goods_price }} 爱心积分</template> -->
          <template v-if="brand_id !== 6">
            {{ $i18n.t("money") }}
      ...
      <!-- 我的订单   点击进去 联系客服 -->
      <!-- <div class="kefu" v-if="order_data.has_many_order_goods && order_data.has_many_order_goods.length > 0" @click="getKefu" >
    ...
    <div class="detail_pay">
      <template v-for="(item, index) in order_data.button_models">
        <div v-if="index < 2" :key="index" class="order_delete" :class="[!item.api ? 'noClickBG' : '', item.value == 1 ? 'red-button' : '']" @click="operation(item, order_data)">
          {{ item.name }}
        </div>
        <div :key="index" v-if="index == 2 && order_data.button_models.length > 2" class="order_delete red-button" @click.stop="showMore(order_data.order_sn)">更多</div>
        ...
        <div class="more-button" :key="index" v-if="index == 2 && showID == order_data.order_sn">
          <p v-for="(item, ind) in order_data.button_models" :key="ind" v-if="ind >= 2" @click="operation(item, order_data)" :class="[!item.api ? 'noClickBG' : '']">
            {{ item.name }}
          </p>
        </div>
      </template>
