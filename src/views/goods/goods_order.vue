<template>
  <div id="goodsinfo" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
          ...
          <div class="addr" v-if="is_open_store_delivery" style="text-align: left">
            <div class="detail-item" style="width: 100%">
              ...
              <div style="display: flex; justify-content: flex-end">
                <div @click="showDeliveryMap" style="text-align: center; width: 5rem; border-radius: 2rem; border: 1px solid rgb(204, 204, 204); padding: 0.25rem">配送范围</div>
              </div>
            </div>
          </div>
        </template>
    ...
    <div class="goods-detail">
      ...
      <div v-for="(item, index) in order_data" :key="index">
        ...
        <div class="detail_good w">
          <div class="goods" v-for="(g, i) in item.order_goods" :key="i">
            ...
            <div class="warp" v-if="!isRent">
              ...
              <div class="price">
                <p v-show="typeOfMerchandise == 7">{{ $i18n.t("money") }}{{ (g.price / g.total).toFixed(2) }}</p>

                <p v-show="typeOfMerchandise != 7">{{ (g.price / g.total).toFixed(0) }}</p>
        ...
        <div class="tbs">
          ...
          <!-- <div class="num list">
            <div class="left  ss">{{ isRent ? "租金" : "商品" }}</div>
          ...
          <div class="num list">
            <!-- <div class="left" v-show="typeOfMerchandise==5">{{ isRent ? "租金" : "爱心积分" }}</div> -->
            <div class="left" v-show="typeOfMerchandise == 6">{{ isRent ? "租金" : "通兑" }}</div>
            <div class="left" v-show="typeOfMerchandise == 5 || typeOfMerchandise == 7">{{ isRent ? "租金" : "金额" }}</div>

            <div class="right">
              <font color="#E84E40" v-show="typeOfMerchandise != 7">{{ Number(item.order_goods_price).toFixed(0) }}</font>
              <font color="#E84E40" v-show="typeOfMerchandise == 7">{{ item.order_goods_price }}</font>
            </div>
          </div>
          ...
          <div class="freight-num list">
            ...
            <div class="right">
              <font color="#E84E40"> <span> &nbsp;</span>{{ Number(item.dispatch_price).toFixed(0) }} </font>
            </div>
          </div>
          <!-- <div class="freight-num list">
            <div class="left">运费:</div>
          ...
          <div class="score list" v-if="item.order_deductions">
            <div v-for="(d, i) in item.order_deductions" :key="i" style="width: 100%; display: block; clear: both; margin-bottom: 2.1875rem">
              <div class="left">
                可用{{ parseFloat(d.coin).toFixed(0) }}通兑{{ d.name == "积分" ? integral : d.name }}
    ...
    <van-popup v-model="show1" position="bottom" :style="{ height: '80%' }" class="pc_popup">
      <div class="popup_box">
        ...
        <div class="type_box">
          <h2>发票抬头</h2>
          <div class="type">
            <button type="button" :class="{ cur: iscur }" @click="invoice_title">个人</button>
            <button type="button" :class="{ cur: iscur_b }" @click="invoice_title">单位</button>
          </div>
    ...
    <div class="tbs" style="margin-top: 0.625rem">
      <!-- 订单总金额 飞 -->
      <!-- <div class="price list s" v-for="item in total_items" :key="item.code">
        <div class="left ">{{ item.name }}</div>
        <div class="right">
      ...
      <div class="price list s" v-for="item in total_items" :key="item.code">
        <div class="left" v-show="typeOfMerchandise == 6">{{ item.name == "订单总金额" ? "订单总通兑" : "总运费" }}</div>
        <!-- <div class="left " v-show="typeOfMerchandise==5">{{ item.name=='订单总金额'?'订单总爱心积分':'总运费' }} </div> -->
        <div class="left" v-if="item.name != '总抵扣'" v-show="typeOfMerchandise == 5 || typeOfMerchandise == 7">{{ item.name == "订单总金额" ? "订单总金额" : "总运费" }}</div>
        <div class="left" v-if="item.name == '总抵扣'" v-show="typeOfMerchandise == 5 || typeOfMerchandise == 7">{{ item.name}}</div>

        <div class="right">
          <!-- <font color="#E84E40"><span v-if="item.code == 'total_deduction_price'">-</span>{{ (Number(item.amount)).toFixed(0) }}</font> -->
          <font color="#E84E40"><span v-if="item.code == 'total_deduction_price'">-</span>{{ Number(item.amount) }}</font>
        </div>
      </div>
    ...
    <div class="detail_pay">
      <ul class="total">
        ...
        <li>
          合计：<font style="color: #f15353">
            <span class="span_t" v-show="typeOfMerchandise != 7">{{ Number(price).toFixed(0) }}</span>
            <span class="span_t" v-show="typeOfMerchandise == 7">{{ price }}</span>
          </font>
        </li>
      </ul>
      <div class="order_delete s" @click="submit" :total_items="total_items">提交订单</div>
    </div>
      ...
      <div class="add-info">
        <ul>
          <li v-for="(item, i) in addressData" :key="i" @click="selectAddress(item)">
            ...
            <p class="edit_btn" v-if="item.isdefault" @click="editAddress">编辑</p>
