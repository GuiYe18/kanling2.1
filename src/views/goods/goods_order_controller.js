...
export default {
  data() {
    return {
      typeOfMerchandise: '',  //订单提交页面  做通兑爱心判断   父组件传过来的值
      popCode: "",
      showPop: false,
      stepper_show: 0,
      clicktag: 0,
  ...
  async activated() {
    ...
    if (_root_tag == TAG_ACTION_BUY || _root_tag == "appointment_goods" || _root_tag == "yun_sign_goods" || _root_tag == "activity" || _root_tag == "cps" || _root_tag == "channel_buy") {
      ...
      // ppppppp = this.$route.query.iddd;
      // console.log('ppppppp',this.$route.query.iddd)
      // alert('ppppppp',this.$route.query.iddd)
      this.typeOfMerchandise = this.$route.query.iddd
  ...
  methods: {
    ...
    // // 测试
    // submit(){
    //   console.log('total_items',this.typeOfMerchandise,this.total_items.amount);

    // },
    // 普通商品提交订单
    submit() {
      // 判断积分是否充足   5 总爱心积分    6 总通兑    7总金额
      // 订单积分          this.total_items.amount
