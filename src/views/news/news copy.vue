<template>
  <div class="content">
    <van-badge v-if="num" class="newsNum" :content="num" />
    <van-tabs v-model="active" color="#0579fc" @change="changeTab(active)" swipeable>
      <van-tab title="关注">
        <van-list class="followListBg" v-model="followLoading" :finished="followFinished" finished-text="没有更多了" @load="followOnLoad">
          <van-cell v-for="followItem in followList" :key="followItem.id" @click="tofollowDetailHandle(followItem)">
            <div class="followItem">
              <div class="photo"><img :src="followItem.topurl" alt="" /></div>
              <div class="followInfo">
                <div class="followName">{{ followItem.qsn_name }}</div>
                <div class="followAddress">
                  <span> <van-icon color="#999" class="addressIcon" name="location" />{{ followItem.sqnadress }} </span>
                </div>
              </div>
            </div>
          </van-cell>
