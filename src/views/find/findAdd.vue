<template>
  <div id="income">
    <div class="supplier">
      ...
      <div class="main">
        <van-field v-model="formData.content" rows="6" autosize type="textarea" maxlength="300" placeholder="请输入想要发布的内容…" show-word-limit />
        <div class="upload-wrap">
          <van-uploader v-model="detailFileList" upload-icon="plus" multiple :max-count="9" :after-read="afterReadDetail" @delete="handleDeleteDetail" />
        </div>
        <van-cell value="视频" :border="false" />
        <div class="upload-container">
          <div class="upload-item">
            <van-icon v-if="formData.videourl" name="delete" @click="handleDeleteVideo" />
            <video v-if="formData.videourl" :src="formData.videourl" controls disablePictureInPicture preload="none" controlslist="nodownload"></video>
        ...
        <div class="field-item">
          ...
          <!-- <label>所在位置</label> -->
          <div class="wrap">
            <input class="text-left" type="text" v-model="formData.sqnadress" readonly placeholder="当前位置获取中..." />
          </div>
        </div>
        <div class="field-item">
        ...
        <div class="btn-group">
          ...
          <div class="btn-wrap">
            <div>
              <button v-for="item in typeList" :key="item.type" :class="{ selected: item.type === formData.type }" @click="selectType(item)">
                {{ item.title }}
              </button>
            </div>
          </div>
...
export default {
  ...
  activated() {
    ...
    // 获取发布时选中的值
    this.formData.type = this.$route.query.typesss
      ...
      axios({
        ...
        if (res.data.result === 1) {
          ...
          if (this.isWechat) {
            ...
            const payURL = data.mweb_url + "&redirect_url=" + encodeURIComponent(this.url + "&out_trade_no=" + data.out_trade_no);
