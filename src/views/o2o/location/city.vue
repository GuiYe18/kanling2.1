<template>
  <div>
    <div id="position">
      <div class="left" @click="showCitys">
    ...
    <ul class="address-list">
      <li @click="goToSelect(item)" class="address-list-item" v-for="(item, index) in addressList" v-show="item.location" :key="index">{{ item.name }} --- {{ item.address }}</li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    goToSelect(item) {
      ...
      // addcode存在vuex里起来
      this.$store.commit("adcode", item.adcode);
