<template>
  <div id="all">
    ...
    <div class="cover" v-if="!isDecorate">
      <div v-show="advertisement.rule === 0 && advertisement.switch && fullShow" class="fullScreen">
        <div class="full-image">
          <img :src="advertisement.image" />
          <span class="close"><span class="skip" @click="close">跳过</span>{{ advertisement.time }}</span>
        </div>
      </div>
      <div class="mask" v-show="advertisement.Midswitch">
        <div class="mask-blank">
          <div class="pop" v-if="advertisement.Midrule === 0 && advertisement.Midswitch && popShow">
            <div class="pop-image">
              <img :src="advertisement.Midimage" @click="turnTo" />
    ...
    <div :class="{ scroll: true }" v-show="!fullShow">
      ...
      <van-popup v-model="posterShow" closeOnClickModal="true" style="padding: 0; background: none">
        ...
        <div style="width: 100%; height: 100%; overflow: hidden" class="code_box">
          <div style="width: 100%">
            <img :src="follow_mode.follow_img" v-if="follow_mode.follow_img" style="width: 100%; border-radius: 0.375rem" />
          </div>
          <!-- 关闭图标 -->
      ...
      <div class="home" v-if="$store.state.temp.default">
        <div :class="{ search: 'ture', mout: amout, isfollow }" id="d1" :style="{ width: fun.getPhoneEnv() == 3 ? '375px' : '100%' }">
        ...
        <div id="home">
          <div class="banner">
            ...
            >
              <div v-for="(slide, index) in $store.state.temp.default.ads" :key="index" @click="bannerClick(slide)" :style="{ height: fun.getPhoneEnv() == 3 ? '9.875rem' : '38vw' }">
                <img width="100%" height="100%" :src="slide.thumb" />
              </div>

              <div :style="{ height: fun.getPhoneEnv() == 3 ? '9.875rem' : '38vw' }" v-if="this.fun.isTextEmpty($store.state.temp.default.ads)">
                <img style="width: 100%; height: 100%" src="../../assets/images/loading.jpg" />
              </div>
            </c-myswipe>
          </div>
            ...
            <div class="top">
              ...
              <ul class="right">
                <li @click="gotoAdvs($store.state.temp.default.advs[1].link)">
                  <img :src="$store.state.temp.default.advs[1] ? $store.state.temp.default.advs[1].img : require('../../assets/images/loading.jpg')" alt="" style="width: 100%; height: 100%" />
                </li>

                <li @click="gotoAdvs($store.state.temp.default.advs[2].link)">
                  <img :src="$store.state.temp.default.advs[2] ? $store.state.temp.default.advs[2].img : require('../../assets/images/loading.jpg')" alt="" style="width: 100%; height: 100%" />
                </li>
              </ul>
            </div>
            <div class="bottom_img">
              <ul>
                <li @click="gotoAdvs($store.state.temp.default.advs[3].link)">
                  <img :src="$store.state.temp.default.advs[3] ? $store.state.temp.default.advs[3].img : require('../../assets/images/loading.jpg')" alt="" style="width: 100%; height: 100%" />
                </li>

                <li @click="gotoAdvs($store.state.temp.default.advs[4].link)">
                  <img :src="$store.state.temp.default.advs[4] ? $store.state.temp.default.advs[4].img : require('../../assets/images/loading.jpg')" alt="" style="width: 100%; height: 100%" />
                </li>

                <li @click="gotoAdvs($store.state.temp.default.advs[5].link)">
                  <img :src="$store.state.temp.default.advs[5] ? $store.state.temp.default.advs[5].img : require('../../assets/images/loading.jpg')" alt="" style="width: 100%; height: 100%" />
                </li>

                <li @click="gotoAdvs($store.state.temp.default.advs[6].link)">
                  <img :src="$store.state.temp.default.advs[6] ? $store.state.temp.default.advs[6].img : require('../../assets/images/loading.jpg')" alt="" style="width: 100%; height: 100%" />
                </li>
              </ul>
            </div>
          </div>
          ...
          <div id="brand-box" v-if="!this.fun.isTextEmpty($store.state.temp.default.brand)">
            ...
            <ul>
              <li v-for="(item, i) in $store.state.temp.default.brand" :key="i" @click="gotoBrand(item)">
          ...
          <div id="limit-time" v-if="!this.fun.isTextEmpty($store.state.temp.default.time_goods)">
            ...
            <div class="goods-box">
              <div class="goods" v-for="(item, i) in $store.state.temp.default.time_goods" :key="i" @click="gotoTimeGood(item.id)">
                <div class="left">
                ...
                <ul>
                  ...
                  <li class="price">
                    {{ $i18n.t("home.限时价") }}：{{ $i18n.t("money") }}<span>{{ item.vip_level_status && item.vip_level_status.status == 1 ? item.vip_level_status.word : item.price }}</span>
                    <font
                      ><span v-show="parseInt(item.market_price)">{{ $i18n.t("money") }}{{ item.market_price }}</span></font
                    >
                  </li>
                  <li class="time">
                    <i class="iconfont icon-member-time" style="font-size: 1.625rem; color: #f96587"></i>
                    <!-- <span>还剩2天20:00:59</span> -->
                    <van-count-down :time="fun.getTimeDifference(fun.getTimestamp(item.has_one_goods_limit_buy.end_time))" class="bottom_time" format="DD天HH时mm分ss秒">
                      <template #default="timeData">
                        <em style="padding: 0.125rem; border-radius: 0.125rem">{{ timeData.days < 10 ? `0${timeData.days}` : timeData.days }}</em
                        >{{ $i18n.t("天") }} <em style="padding: 0.125rem; border-radius: 0.125rem">{{ timeData.hours < 10 ? `0${timeData.hours}` : timeData.hours }}</em
                        ><b>:</b> <em style="padding: 0.125rem; border-radius: 0.125rem">{{ timeData.minutes < 10 ? `0${timeData.minutes}` : timeData.minutes }}</em
                        ><b>:</b>
          ...
          <div id="pop-recommend" v-if="!this.fun.isTextEmpty($store.state.temp.default.goods)">
            ...
            <div class="goodsList" v-if="$store.state.temp.default && $store.state.temp.default.goods">
              <c-goodsList :goods="$store.state.temp.default.goods.data" :last_page="$store.state.temp.default.goods.last_page" :isDefault="1" :text="$i18n.t('home.推荐商品')"></c-goodsList>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="temp" v-if="$store.state.temp.item && !isDecorate">
        ...
        >
          <div class="search_box3" @click="tosearch">
            <input type="text" :title="$i18n.t('搜索')" readonly="readonly" :placeholder="sliderNav.params.searchword" />
            <i class="iconfont icon-sousuo1"></i>
          </div>
        ...
        <div class="home" v-if="$store.state.temp.item.pageinfo">
          ...
          <template v-if="$store.state.temp.item.pageinfo.temp == 'topbar'">
            ...
            >
              <a v-if="isMiniApp" @click="this.fun.gotoMiniAppCs"> <img :src="$store.state.temp.item.pageinfo.params.floatimg" style="height: 100%; width: 100%" /></a>
              <a v-if="!isMiniApp" :href="$store.state.temp.item.pageinfo.params.floathref"><img :src="$store.state.temp.item.pageinfo.params.floatimg" style="height: 100%; width: 100%" /></a>
            </div>
          </template>
          <!-- 旧装修的浮动按钮end -->
          ...
          <div class="home" v-if="$store.state.temp.item.data && !isInDiy">
            <template v-for="(item, index) in $store.state.temp.item.data">
              <component :key="index" :is="item.temp" :datas="item" :home="true" :tag="123" :index="index" :all="$store.state.temp.item.data.length"> </component>
            </template>
      ...
      <div v-if="isDecorate && !fullShow && showPage">
        ...
        <template v-for="(item, index) in components">
          <component
            ...
            :id="item.id"
            :page_id="page_id"
            :component_key="item.component_key + 2"
