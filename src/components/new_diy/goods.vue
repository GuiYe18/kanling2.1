  ...
  <div :class="className" :style="{ backgroundColor: datas.preview_color }">
    ...
    <div class="u_goodslist" v-if="datas.preview_color">
      <div class="u_goodslist_two" v-if="datas.sort_style.choose_icon == 1">
        ...
        <div class="u_goodslist_two_child big u_goodslist_two_child_only" v-for="(item, index) in datas.list.data ? datas.list.data : datas.list" :key="index" @click.stop.prevent="gotoDetail(item)">
          <template v-if="(item && item.stock_status === 0) || datas.disable_good != 0">
            ...
            >
              <!-- 限时抢购 -->
              <img class="ssss" src="../../assets/images/new_diy/xianshiqianggou@2x.png" v-if="datas.good_tag.choose_icon == 5" alt="" />
              <!-- 包邮 -->
            ...
            <div class="twoChild_bottom">
              ...
              <div class="Child_price">
                <div class="Child_now_price" :style="{ width: datas.button_type == 0 ? '100%' : '70%' }">
                  ...
                  >
                  <em class="oneChild_original_price" v-if="datas.content_list.includes('2') && item.priceold != 0 && item.priceold != item.pricenow"
      ...
      <div class="u_goodslist_three" v-if="datas.sort_style.choose_icon == 2">
        ...
        <!-- 双排样式 -->
        <div class="u_goodslist_three_child" v-for="(item, index) in datas.list.data ? datas.list.data : datas.list" :key="index" @click.stop.prevent="gotoDetail(item)">
          <template v-if="(item && item.stock_status === 0) || datas.disable_good != 0">
            <div class="Failure" v-if="datas.disable_good != 0 && item.stock_status && item.stock_status != 0" :class="[datas.disable_good == 1 ? 'failure_cover' : '']">
              <div v-if="item.stock_status == 1" class="Failure_text">{{ $i18n.t("售罄") }}</div>
              <div v-if="item.stock_status == 2" class="Failure_text">{{ $i18n.t("已下架") }}</div>
            ...
            <div class="threeChild_b">
              ...
              <div class="Child_price">
                <div
                  ...
                  :style="{ width: datas.button_type == 1 || datas.button_type == 2 ? '' : '80%', fontSize: item.vip_level_status && item.vip_level_status.status == 1 ? '12px' : '' }"
                  v-if="(datas.content_list.includes('3') && !datas.content_list.includes('8')) || (datas.content_list.includes('8') && item.coupon && item.coupon.coupon_method === 0)"
                >
                ...
                >
                  <em v-if="datas.show_price_text">券</em>
                  <em>{{ $i18n.t("money") }}</em>
              ...
              <div class="oneChild_original_price" v-if="datas.content_list.includes('2') && item.priceold != 0 && item.priceold != item.pricenow">
      ...
      <div class="u_goodslist_one" v-if="datas.sort_style.choose_icon == 3">
        ...
        <!-- 横排样式 -->
        <div class="u_goodslist_one_child" v-for="(item, index) in datas.list.data ? datas.list.data : datas.list" :key="index" @click.stop.prevent="gotoDetail(item)">
          <template v-if="(item && item.stock_status === 0) || datas.disable_good != 0">
            <div class="Failure" v-if="datas.disable_good != 0 && item.stock_status && item.stock_status != 0" :class="[datas.disable_good == 1 ? 'failure_cover' : '']">
              <div v-if="item.stock_status == 1" class="Failure_text">{{ $i18n.t("售罄") }}</div>
              <div v-if="item.stock_status == 2" class="Failure_text">{{ $i18n.t("已下架") }}</div>
            ...
            <div class="oneChild_right">
              <div class="buttons_box">
                <div
                  v-if="$store.state.temp.plugin.goods_show && $store.state.temp.plugin.goods_show.goods_group && $store.state.temp.plugin.goods_show.goods_group.includes('collect')"
                ...
                <div
                  v-if="$store.state.temp.plugin.goods_show && $store.state.temp.plugin.goods_show.goods_group && $store.state.temp.plugin.goods_show.goods_group.includes('share')"
                  class="share_buton"
      ...
      <div class="u_goodslist_four" v-if="datas.sort_style.choose_icon == 4">
        ...
        <!-- 三排排序样式 -->
        <div class="u_goodslist_four_child" v-for="(item, index) in datas.list.data ? datas.list.data : datas.list" :key="index" @click.stop.prevent="gotoDetail(item)">
          <template v-if="(item && item.stock_status === 0) || datas.disable_good != 0">
            <div class="Failure" v-if="datas.disable_good != 0 && item.stock_status && item.stock_status != 0" :class="[datas.disable_good == 1 ? 'failure_cover' : '']">
              <div v-if="item.stock_status == 1" class="Failure_text">{{ $i18n.t("售罄") }}</div>
              <div v-if="item.stock_status == 2" class="Failure_text">{{ $i18n.t("已下架") }}</div>
      ...
      <div class="u_goodslist_five" v-if="datas.sort_style.choose_icon == 5" @scroll="orderScroll">
        <div v-if="(datas.list.data && datas.list.data.length <= 0) || datas.list.length <= 0" style="width: 100%; height: 150px; text-align: center">
          <img src="../../assets/images/blank.png" alt="" style="width: 5rem; margin: 2rem 0" />
        </div>
        <!-- 三排滑动样式 -->
      ...
      <div class="u_goodslist_six" v-if="datas.sort_style.choose_icon == 6">
        ...
        <!-- 一大两小 -->
        <template v-for="(item, index) in datas.list.data ? datas.list.data : datas.list">
          <template v-if="(item && item.stock_status === 0) || datas.disable_good != 0">
...
export default {
      ...
      if (this.tabcontrol) {
        ...
        if (newVal && this.isLast && this.datas.sort_style.choose_icon != 5 && this.isLoadMore) {
          ...
          if (this.alreadyHandleArray.slice(12 * this.arrIndex).length > 0) {
            this.datas.list.data = this.datas.list.data.concat(this.alreadyHandleArray.slice(12 * this.arrIndex, 12 * (this.arrIndex + 1))); //数组拼接
  ...
  mounted() {
    ...
    console.log("datasdatasdatasdatasdatasdatasdatasdatasdatasdatasdatas", this.datas);
  ...
  methods: {
    ...
    //     console.log('222222222222222',realVal.split('.'));
    //     var newRealVal = realVal.split('.')[0]
    //     if (realVal.split('.')[1]>40){
    //         console.log('sssssss',realVal.split('.')[1]);
    //         newRealVal+1
    //     }
    ...
    //商品跳转
    gotoDetail(good) {
      console.log("goodgoodgoodgoodgoodgoodgoodgoodgoodgood", good);
      let url = good.plugin_id == 69 ? "grab_group" : "goods";
      this.$router.push(this.fun.getUrl(url, { id: good.id }));
    },
    // //捐款跳转
    // gotoDonation() {
    //   console.log('this.fun.getUrl',this.fun.getUrl);
    //   // this.$router.push(this.fun.getUrl('www'));
    // },
    orderScroll() {
