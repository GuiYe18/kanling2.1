        ...
        <ul v-if="datas.show_list.length > 0">
          <template v-for="(item, index) in datas.show_list">
            ...
            <li v-if="item.value === 'extension' && member_item.integral_show" @click="goUrl(item.value)" :key="index">
              <span
                ><span>{{ $i18n.t("money") }}</span
                >{{ member_item.income || "0.00" }}</span
              >
              <span>{{ name_of_withdrawal_text }}</span>
            </li>
          </template>
          <!-- <li :key="index">
            <span>{{ love_integral }}</span>
            <span>爱心s积分</span>
          </li> -->
        </ul>
      </div>
...
export default {
  props: ["id", "datas", "member_item", "love_integral"],
  data() {
    return {
  ...
  computed: {
    css() {
      // console.log("datas", this.love_integral);
