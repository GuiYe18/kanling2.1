  ...
  <div id="orderList">
    <van-checkbox-group v-model="checkList" @change="multiplePayChange">
      <div v-for="(order, index) in datasource" :class="orderType == 'lease_toy' && index == datasource.length - 1 ? 'Margintop shop' : 'shop'" :key="index">
        <div class="title">
          <van-checkbox checked-color="#f15353" :name="order.id" v-if="status == 1 && !notShowCheck">&nbsp;</van-checkbox>
          <h4 style="display: flex; word-break: keep-all">
            <em @click="decrease">订单号：{{ order.order_sn }}</em>
        ...
        <div class="allbt s" v-if="order.button_models.length > 0">
          {{ i }}
          <template v-for="(item, i) in order.button_models">
            <span :key="i" v-if="i < 2" @click="operation(item, order, order.barter)" :class="[!item.api ? 'noClickBG' : '', item.value == 1 ? 'red-button' : '']">{{ item.name }}</span>
            <span :key="i" v-if="i == 2 && order.button_models.length > 2" class="red-button" @click.stop="showMore(order.order_sn)">更多</span>
            <div class="trans" :key="i" v-if="i == 2 && showID == order.order_sn"></div>
            <div class="more-button" :key="i" v-if="i == 2 && showID == order.order_sn">
              <p v-for="(item, i) in order.button_models" :key="i" v-if="i >= 2" @click="operation(item, order, order.barter)" :class="[!item.api ? 'noClickBG' : '']">{{ item.name }}</p>
            </div>
          </template>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    operation(btn, order, barter) {
      ...
      console.log("sssssssss", btn, order, barter);
      if (!btn.api) {
